...
public class GestionProjetAction extends ActionSupport {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 1141919983428921392L;
	...
	public String doCreate() {
		...
		if (this.projet != null) {
			...
			if (this.projet.getResponsable() == null || this.projet.getResponsable().getId() == null) {
				this.addFieldError("projet.responsable.id", "ne doit pas être vide");
			} else {
				try {
					Utilisateur vResponsable = WebappHelper.getManagerFactory().getUtilisateurManager()
		...
		// Si on doit aller sur le formulaire de saisie, il faut ajouter les info
		// nécessaires
